...
import { MATERIALS, getDbmValue, getSignalQuality } from "../controlador";
...
export default function NetworkGrid({
  ...
  const downloadGridImage = () => {
    ...
    const filename = `malla_wifi_espectro_laplace_${timestamp}.png`;
    ...
    fetch("/api/guardar-resultado", {
      ...
      .then(res => res.json())
      .then(data => console.log("Resultado de imagen guardado en el servidor:", data))
      .catch(err => console.error("Error al guardar imagen de simulación en servidor", err));
  };
  ...
  return (
    <div className={`flex flex-col h-auto rounded-3xl border p-6 justify-between transition-all duration-300 ${cardBgClass}`}>
            ...
            row.map((cell, cIdx) => {
              ...
              return (
                <div
                  ...
                  // Las clases de CSS se adaptan según el estado y la herramienta seleccionados. 
                  // Si 'move' está activo y la celda contiene un router, mostramos un cursor-grab
                  // y un borde de enfoque añil ('border-indigo-500 border-2') con escalado y sombra para feedback.
                  className={`w-full aspect-square flex items-center justify-center rounded-sm transition-all duration-100 border-[0.5px] border-slate-200/5 text-[8px] sm:text-[9px] font-mono select-none relative ${
                    ...
                    isBoundary 
                      ? outerBoundaryBg 
                      : (selectedTool === "move" && isRouter)
                        ? "cursor-grab active:cursor-grabbing hover:scale-115 hover:z-25 border-indigo-500 border-2 shadow-lg shadow-indigo-500/20"
                        : "cursor-pointer hover:scale-110 hover:z-10"
                  }`}
                >
                  {isRouter ? (
                    <div className="relative flex items-center justify-center w-full h-full">
                      {/* Anillos concéntricos de emisión electromagnética que pulsan dinámicamente */}
                      <span className="absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-70 animate-ping duration-1000" />
                      {/* Si la herramienta de movimiento 'Move' está activa, mostramos una aureola añil adicional */}
                      {selectedTool === "move" && (
                        <span className="absolute inline-flex h-[140%] w-[140%] rounded-full bg-indigo-500/40 opacity-50 animate-pulse" />
                      )}
                      {/* Icono de enrutador con oscilaciones en rebote si se puede mover, o pulsaciones suaves en modo normal */}
                      <Router className={`w-4.5 h-4.5 sm:w-5.5 sm:h-5.5 text-white relative z-10 ${selectedTool === "move" ? "animate-bounce" : "animate-pulse"}`} />
                    </div>
                  ) : cell.type === "wall" ? (
                    // Mostrar la inicial del material aislante sobre la celda (Y: Yeso, L: Ladrillo, C: Concreto, M: Metal)
                    ...
                    // Si el usuario activa 'Mostrar valores numéricos', renderizamos el potencial potencial escalar de señal Wi-Fi
                    <span className={textContrastClass}>
                      {Math.round(cell.val)}
                    </span>
                  ) : null}

                  {/* Insignia indicadora de candidatos óptimos de ubicación de enrutador Wi-Fi (Laplace K-Means) */}
                  {!isBoundary && candIndex !== undefined && candIndex !== -1 && (
                    ...
                    >
                      {candIndex === 0 ? (
                        // El candidato número 1 (óptimo absoluto) posee aureolas doradas más anchas y animación bounce
                        ...
                        // Candidatos de cobertura alternativos numerados (puntos 2, 3, etc.)
      ...
      <div className={`min-h-[56px] border-t mt-4 pt-3 flex flex-wrap items-center justify-between text-xs gap-4 ${
        ...
        {hoveredCell ? (
              ...
              <div className="flex flex-wrap items-center justify-between w-full gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-slate-550 dark:text-slate-400">Tipo:</span>
                  ...
                  {(() => {
                    ...
                    return distMeters !== null && !isBoundary && hoveredCell.type !== "router" ? (
                      <div className="hidden sm:flex items-center gap-1.5 border-l border-slate-205 dark:border-slate-800 pl-3">
                        <span className="text-slate-550 dark:text-slate-400">Distancia:</span>
                ...
                {!isBoundary && (
                  <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-550 dark:text-slate-400">Potencia:</span>
                    ...
                    <div className="flex items-center gap-1.5 border-l border-slate-200 dark:border-slate-800 pl-3">
                      <span className="text-slate-550 dark:text-slate-400">Pérdida/m:</span>
                      <span className={`font-mono font-bold ${isDarkMode ? "text-slate-200" : "text-slate-800"}`}>
          ...
          <div className="flex items-center justify-center w-full text-slate-500 dark:text-slate-400 py-1 gap-2">
