...
import { buildInitialGrid, runSolverStep, solveInstant, MATERIALS, getDbmValue, getSignalQuality, ROUTER_MODELS } from "./solver";
...
export default function App() {
  ...
  // Finds the best router location using quick physical-Laplace sampling
  const findBestRouterLocation = () => {
    ...
    setTimeout(() => {
      ...
      // Choose stride based on grid density to guarantee instant execution
      ...
      for (let r = 2; r < rows - 2; r += step) {
        for (let c = 2; c < cols - 2; c += step) {
          ...
          // Speed solving with 25 relaxation iterations is highly accurate for potential comparison
          ...
          // Composite evaluation score balancing average strength (70%) and coverage factor (30%)
      ...
      // Exclude points that are very close to each other to give 4 geographically distinct candidates
  ...
  const applyBestRouterLocation = (customX?: number, customY?: number) => {
    ...
    // Automatically solve the propagation for gorgeous instant rendering
    ...
    setIsConverged(result.finalError < config.tolerance);
    setCurrentErrorHistory(result.errorHistory);
    setOptimizationResult(null); // Reset suggestion card once successfully applied 
  };
